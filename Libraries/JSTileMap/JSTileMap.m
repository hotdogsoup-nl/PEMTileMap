...
#if TARGET_OS_OSX
  [aCoder encodeSize:NSSizeFromCGSize(_mapTileSize) forKey:@"TMXLayerTileSize"];
#else
...
#if TARGET_OS_OSX
      _mapTileSize = NSSizeToCGSize([aDecoder decodeSizeForKey:@"TMXLayerTileSize"]);
#else
...
#if TARGET_OS_OSX
    [aCoder encodeSize:NSSizeFromCGSize(_layerGridSize) forKey:@"TMXLayerInfoGridSize"];
    [aCoder encodePoint:NSPointFromCGPoint(_offset) forKey:@"TMXLayerInfoOffset"];
#else
...
#if TARGET_OS_OSX
      _layerGridSize = NSSizeToCGSize([aDecoder decodeSizeForKey:@"TMXLayerInfoGridSize"]);
        _offset = NSPointToCGPoint([aDecoder decodePointForKey:@"TMXLayerInfoOffset"]);
#else
...
#if TARGET_OS_OSX
    [aCoder encodePoint:NSPointFromCGPoint(_positionOffset) forKey:@"TMSObjectGroupPosOffset"];
#else
...
#if TARGET_OS_OSX
      _positionOffset = NSPointToCGPoint([aDecoder decodePointForKey:@"TMSObjectGroupPosOffset"]);
#else
...
#if TARGET_OS_OSX
    [aCoder encodeSize:NSSizeFromCGSize(_tileSize) forKey:@"TMXTilesetTileSize"];
    [aCoder encodeSize:NSSizeFromCGSize(_unitTileSize) forKey:@"TMXTilesetUnitTileSize"];
      [aCoder encodeSize:NSSizeFromCGSize(_imageSize) forKey:@"TMXTilesetImageSize"];
#else
...
#if TARGET_OS_OSX
      _tileSize = NSSizeToCGSize([aDecoder decodeSizeForKey:@"TMXTilesetTileSize"]);
      _unitTileSize = NSSizeToCGSize([aDecoder decodeSizeForKey:@"TMXTilesetUnitTileSize"]);
        _imageSize = NSSizeToCGSize([aDecoder decodeSizeForKey:@"TMXTilesetImageSize"]);
#else
...
#if TARGET_OS_OSX
    [aCoder encodeSize:NSSizeFromCGSize(_mapSize) forKey:@"JSTileMapMapSize"];
    [aCoder encodeSize:NSSizeFromCGSize(_tileSize) forKey:@"JSTileMapTileSize"];
#else
...
-(instancetype)initWithCoder:(NSCoder *)aDecoder
{
  ...
  {
#if TARGET_OS_OSX
      _mapSize = NSSizeToCGSize([aDecoder decodeSizeForKey:@"JSTileMapMapSize"]);
      _tileSize = NSSizeToCGSize([aDecoder decodeSizeForKey:@"JSTileMapTileSize"]);
#else
...
-(void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict
{
	...
	else if([elementName isEqualToString:@"image"])
	{
		...
		else if (self.parentElement == TMXPropertyMap)
		{
            ...
            NSLog(@"Tile image collections unsupported!");
            [parser abortParsing];
            return;
        }
	}
...
int InflateMemoryWithHint(unsigned char *in, unsigned int inLength, unsigned char **out, unsigned int outLengthHint )
{
    ...
    if (err != Z_OK || *out == NULL) {
        if (err == Z_MEM_ERROR)
            NSLog(@"Out of memory while decompressing map data!");
        
        else if (err == Z_VERSION_ERROR)
            NSLog(@"Incompatible zlib version!");
        
        else if (err == Z_DATA_ERROR)
            NSLog(@"Incorrect zlib compressed data!");
        
        else
            NSLog(@"Unknown error while decompressing map data!");
